...
def normalize_sasb_report(sasb_report: str) -> str:
    """
    build_database.py uses metric filenames such as EM-RM.json, so the
    generated database directory is named EM-RM.
    ...
    Return:
      - EM-RM
    """
    return os.path.splitext(os.path.basename(sasb_report))[0].replace("SASB-", "")
...
def build_ocr_path(config, report_folder: str, page: int, ocr_method: str) -> str:
    if ocr_method not in {"olm", "chandra"}:
        raise ValueError(f"Unknown OCR method: {ocr_method}")

    return os.path.join(
        config["paths"]["ocr_output" if ocr_method == "olm" else "chandra_ocr_output"],
        report_folder,
        str(page),
        "output.json",
    )
...
def main(args):
    ...
    # 4. Output CSV
    strategy = "semantic-rerank" if args.rerank else "semantic"
    csv_path = config["paths"]["results_output"].replace("{strategy}", strategy)
        ...
        for item in tqdm(data):
            ...
            if targets:
                ...
                output_dir = os.path.join(
                    config["paths"]["marked_results_output"].replace("{strategy}", strategy),
                    report_folder,
                )
