...
from path_utils import PROJECT_ROOT, load_config
...
def parse_args():
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default=str(PROJECT_ROOT / "prompts" / "prompt_vlm.txt"),
...
def load_metrics(metrics_dir, sasb_report):
    ...
    metrics file:
      EM-RM.json
    """
    base = normalize_sasb_report(sasb_report).replace("SASB-", "")
    ...
    for item in tqdm(data):
        ...
        if err is not None or items is None:
            ...
            for it in items:
                ...
                if bbox_px[0] >= bbox_px[2] or bbox_px[1] >= bbox_px[3]:
                    continue
...
def main():
    ...
    output_dir = Path(config["paths"]["results_output"].replace("{strategy}", "direct")).parent
    output_dir.mkdir(parents=True, exist_ok=True)
    out_json = output_dir / "predictions.json"
    out_raw = output_dir / "raw_responses.json"
    out_csv = output_dir / "prediction.csv"
    out_timing = output_dir / "timing.json"
