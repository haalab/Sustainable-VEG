...
def convert_bbox_to_pdf_points(
    ...
    using the benchmark's top-left-origin PDF coordinate convention.
    """
    if len(bbox_px) != 4:
...
def draw_boxes_on_pdf(
    ...
    """Render a 72-DPI page preview and draw OCR boxes in PDF coordinates."""
    with fitz.open(pdf_path) as document:
        page = document[page_idx - 1]
...
def build_tasks_from_train_test(
    ...
    """Build unique report-page OCR tasks from the benchmark annotations."""
    with open(data_json_path, "r", encoding="utf-8") as handle:
        data = json.load(handle)
    if not isinstance(data, list):
        raise ValueError(f"Dataset {data_json_path} must contain a JSON array")

    pairs_set = set()
    for item in data:
        if not isinstance(item, dict):
            continue
        report = item.get("esg_report")
        page = item.get("page")
        ...
        if page_idx < 1:
            continue
        pdf_path = os.path.join(report_dir, os.path.basename(report))
    ...
    print(f"[MAIN] Built {len(tasks)} unique (pdf, page) tasks from annotations.")
    return tasks

def main():
    print("=== OCR PROCESSING STARTED ===")
    REPORT_DIR = str(PROJECT_ROOT / "data" / "reports")
    DATA_JSON = str(PROJECT_ROOT / "data" / "Sustainable-VEG.json")
    OUTPUT_BASE = str(PROJECT_ROOT / "artifacts" / "ocr" / "olm")
